...
| [`nexolu-auth`](https://github.com/mattchaparro/nexolu-auth) | Identidad centralizada (`auth.nexolu.co`): una sola pantalla de login para todo el ecosistema. Emite aserciones SSO que cada producto verifica **localmente** con la llave publica y canjea por su credencial de siempre | Python, FastAPI, SQLAlchemy async, Alembic | Fase 1 lista: correo + contrasena para el superadmin interno. Los usuarios de los negocios siguen entrando por el login de cada producto |
